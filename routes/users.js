...
const userInit = async (userName, password) => {
  ...
  if (userCheckout) {
    if (userCheckout.password === password) {
...
router.post('/', async (req, res) => {
  try {
    const user = await userInit(req.body.userName, req.body.password);
...
router.get('/userState/:userName', async (req, res) => {
  try {
    ...
    return res.json({ success: true, data: userState });
  } catch (error) {
    console.log(error);
    return res.status(500).json({ success: false, error: 'Server Error' });
  }
...
router.put('/', async (req, res) => {
  try {
    ...
    return res.json({ success: true, data: user.userState });
  } catch (error) {
    console.log(error);
    return res
      .status(500)
